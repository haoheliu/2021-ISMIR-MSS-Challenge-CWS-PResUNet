...
def wiener(
    ...
    * Refinining these initial estimates through a call to
    ...
    if original:
        ...
        # targets_spectrograms:  [shape=(nb_frames, nb_bins, nb_channels, complex, nb_sources)]
        y = targets_spectrograms
    else:
        ...
        print(targets_spectrograms.size(),y.size())
...
def filt_single_channel(mixture_file_path):
    def proc(path):
        ...
        print(audio.size())
        audio_mag = audio.clone() # torch.sqrt(audio[...,0]**2 + audio[...,1]**2)
        audio_mag = audio_mag.permute(2,1,0,3)
        audio = audio.permute(2,1,0,3)
        return audio_mag[...,None], audio

    def inverse(stft, length):
        stft=stft.permute(2,1,0,3)
        wav = torch.istft(stft, n_fft=2048)
        length_current = wav.size()[-1]
        wav = torch.nn.functional.pad(wav,(0,length-length_current))
        return wav

    length,_ = librosa.load(mixture_file_path,mono=True,sr=44100)
    length = length.shape[0]
    _,mixture = proc(mixture_file_path)
    sources = mixture.clone()[...,None]
    out = wiener(targets_spectrograms=sources,mix_stft=mixture,iterations=1,residual=True)
    out = out[...,0]
    out = inverse(out, length)
...
def filt(mixture_file_path, bass_file_path, drums_file_path, other_file_path, vocals_file_path):
    ...
    def inverse(stft, length):
        ...
        wav = torch.nn.functional.pad(wav,(0,length-length_current))
        return wav
    temp,_ = librosa.load(bass_file_path,sr=44100,mono=True)
    length = temp.shape[0]
    bass,_ = proc(bass_file_path)
    drums,_ = proc(drums_file_path)
    vocals,_ = proc(vocals_file_path)
    other,_ = proc(other_file_path)
    sources = torch.cat([bass,drums,vocals,other],dim=-1)
    _,mixture = proc(mixture_file_path)
    out = wiener(targets_spectrograms=sources,mix_stft=mixture,iterations=1)
    bass, drums, vocals, other = divide(out)
    bass, drums, vocals, other = inverse(bass, length),inverse(drums, length),inverse(vocals, length),inverse(other, length)
    sf.write(file=bass_file_path,data=bass.numpy().T,samplerate=44100)
    sf.write(file=drums_file_path,data=drums.numpy().T,samplerate=44100)
