...
class LambdaOverlapAdd(torch.nn.Module):
    ...
    def ola_forward(self, x, type:str, key='wav'):
        ...
        assert x.ndim == 3
        if("vocals" in type):
            self.vocal_cache[type] = {}
        seg_id = type.split("_")[-1]
        ...
        for frame_idx in range(n_chunks):  # for loop to spare memory
            if("vocals" not in type):
                while("vocals_"+seg_id not in self.vocal_cache.keys() or frame_idx not in self.vocal_cache["vocals_"+seg_id]):
                    time.sleep(0.5)
                    # print(type, "wait for frame: ", frame_idx, "vocals_"+seg_id, self.vocal_cache.keys(), [self.vocal_cache[key].keys() for key in self.vocal_cache.keys()])
                    continue

            if(frame_idx == 0):
                if("vocals" not in type):
                    unfolded[..., frame_idx][..., self.in_margin:] = unfolded[..., frame_idx][...,self.in_margin:] - \
                                                                     self.vocal_cache["vocals_" + seg_id][frame_idx]
                frame = self.nnet(unfolded[..., frame_idx][...,self.in_margin:])
                if("vocals" in type):
                    self.vocal_cache[type][frame_idx] = frame[key].clone()
                ...
                if("vocals" not in type):
                    unfolded[..., frame_idx][...,:self.in_margin+last_frame_samples] = unfolded[..., frame_idx][...,:self.in_margin+last_frame_samples] - \
                                                                                       self.vocal_cache["vocals_" + seg_id][frame_idx]
                ...
                if("vocals" not in type):
                    unfolded[..., frame_idx] = unfolded[..., frame_idx] - \
                                                                      self.vocal_cache["vocals_" + seg_id][frame_idx]
    ...
    def forward(self, x, type:str ,key="wav"):
        ...
        with torch.autograd.set_grad_enabled(self.enable_grad):
            olad = self.ola_forward(x,key=key,type=type)
            return olad
